...
// --- ESTRUCTURA DE DATOS ---
// Usaremos vectores para manejar el dinamismo de insertar/eliminar
vector<int> pasos;
vector<int> minutos;
vector<double> calorias; // Arreglo propuesto por el estudiante
...
void ordenarRegistros() {
    ...
    // Bubble sort simple para mantener sincronizados los 3 arreglos
...
void analisisEstadistico() {
    ...
    auto procesar = [](string nombre, vector<double> v) {
        ...
        // Operación Adicional Propuesta: Eficiencia Metabólica (Calorías por minuto)
        // O en este caso, Meta de Salud (Días que superan la media)
        int superanMedia = 0;
        for(double val : v) if(val > media) superanMedia++;
        cout << "\nDias que superan el promedio: " << superanMedia << "\n";
    };

    // Convertimos temporalmente a double para el cálculo genérico
...
double calcularModa(vector<double> v) {
    ...
    return (maxCount > 1) ? moda : -1; // -1 si no hay moda clara
}
